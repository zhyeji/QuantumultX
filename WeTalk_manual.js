...
// ==================== 主流程 ====================
notify('WeTalk调试-开始', '脚本已启动，开始读取存储');

var storeRaw = $prefs.valueForKey(storeKey);
if (!storeRaw) {
  notify('WeTalk调试-失败', '未找到存储数据，请先打开WeTalk触发抓包');
  $done();
}
...
var ids = store.order || [];
if (!ids.length) {
  notify('WeTalk调试-失败', '存储中无账号，请先打开WeTalk触发抓包');
  $done();
}

var acc = store.accounts[ids[0]];
if (!acc || !acc.capture) {
  notify('WeTalk调试-失败', '账号数据不完整');
  $done();
}

notify('WeTalk调试-存储正常', '账号ID: ' + ids[0] + '\n开始查询余额...');
...
$task.fetch({ url: url, method: 'GET', headers: headers }).then(function(res) {
  var info = 'statusCode: ' + res.statusCode + '\n';
  info += 'body前200字符: ' + (res.body || '').substring(0, 200);
  notify('WeTalk调试-查询成功', info);
  $done();
}).catch(function(err) {
  notify('WeTalk调试-查询失败', '错误: ' + (err.error || JSON.stringify(err)));
  $done();
});
