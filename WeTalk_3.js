//2026/04/28 性能优化+金币保留两位小数+内存合并写入
/*
...
function addSignCountTemp(accId) {
  if(!tempSignMap[accId]) tempSignMap[accId] = 0;
  tempSignMap[accId] += 1;
}
function addVideoCountTemp(accId) {
  if(!tempVideoMap[accId]) tempVideoMap[accId] = 0;
  tempVideoMap[accId] += 1;
}

// 获取累计
function getTotalSign(accId) {
  const stat = getDailyStat();
  return (stat.signMap[accId] || 0) + (tempSignMap[accId] || 0);
}
function getTotalVideo(accId) {
  const stat = getDailyStat();
  return (stat.videoMap[accId] || 0) + (tempVideoMap[accId] || 0);
}
...
function runAccount(acc, index, total) {
  ...
  return fetchApi('queryBalanceAndBonus').then(res => {
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) initBalance = formatMoney(d.result.balance);
    } catch (e) {}
    ...
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) lastBalance = formatMoney(d.result.balance);
    } catch (e) {}
