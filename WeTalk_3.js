...
// 获取今日日期 YYYY-MM-DD 修复补零bug，确保日期判断精准
...
// 账号签到计数+自定义增量，默认+1，支持批量一次性累加
function addSignCount(accId, addNum = 1) {
  let stat = getDailyStat();
  if(!stat.signMap[accId]) stat.signMap[accId] = 0;
  stat.signMap[accId] += addNum;
  saveDailyStat(stat);
  return stat.signMap[accId];
}

// 账号视频计数+自定义增量，默认+1，支持批量一次性累加
function addVideoCount(accId, addNum = 1) {
  let stat = getDailyStat();
  if(!stat.videoMap[accId]) stat.videoMap[accId] = 0;
  stat.videoMap[accId] += addNum;
  saveDailyStat(stat);
...
function runAccount(acc, index, total) {
  ...
  // 内存累计本次增量，全程不频繁读写缓存
  let thisRunSignAdd = 0;
  let thisRunVideoAdd = 0;
  // 预读取今日已有累计数，仅1次读缓存
  const todaySignTotal = getSignCount(accId);
  const todayVideoTotal = getVideoCount(accId);
        ...
        setTimeout(() => {
          ...
          fetchApi('videoBonus').then(res => {
            try {
              ...
              // 仅内存累加，不写缓存
              if (d.retcode === 0) thisRunVideoAdd += 1;
            } catch (e) {}
            resolve(next());
          }).catch(() => resolve());
  ...
  return fetchApi('queryBalanceAndBonus').then(res => {
    try {
      const d = JSON.parse(res.body);
      // 强制保留2位小数
      if (d.retcode === 0) initBalance = Number(d.result.balance).toFixed(2);
    } catch (e) {}
    ...
    try {
      const d = JSON.parse(res.body);
      // 仅内存累加，不写缓存
      if (d.retcode === 0) thisRunSignAdd += 1;
    } catch (e) {}
    return doVideoLoop(MAX_VIDEO);
  }).then(() => {
    // 全任务跑完，一次性写入缓存，仅1-2次写操作
    if (thisRunSignAdd > 0) addSignCount(accId, thisRunSignAdd);
    if (thisRunVideoAdd > 0) addVideoCount(accId, thisRunVideoAdd);
    return fetchApi('queryBalanceAndBonus');
  }).then(res => {
    try {
      const d = JSON.parse(res.body);
      // 强制保留2位小数
      if (d.retcode === 0) lastBalance = Number(d.result.balance).toFixed(2);
    } catch (e) {}
    // 读取最终累计总数
    const finalSignTotal = todaySignTotal + thisRunSignAdd;
    const finalVideoTotal = todayVideoTotal + thisRunVideoAdd;
    // 固定对齐格式，完全保留原排版要求
    let line1 = `初始金币：${initBalance} ；最新金币：${lastBalance}`;
    let line2 = `今日签到：${finalSignTotal} 次 ；今日观看：${finalVideoTotal} 条`;
    return line1 + '\n' + line2;
  }).catch(err => {
